...
> 说明：本次审计基于通用 ERP 架构（商品/Item+SKU、保留模块见 `docs/AGENTS.md`）。
> 已删除模块相关的历史审计发现不在本文档列出。
...
- **文件**: `backend/src/repositories/inventory_repo.rs:544-545`
...
- **文件**: `backend/src/services/purchase_service.rs`, `sales_service.rs`, `outbound_service.rs`, `inventory_service.rs`, `contract_service.rs`
...
| Handler 验证 | 所有 POST/PUT handler 均调用 `.validate()` |
...
- **问题**: 使用 `Modal.confirm` 打开商品选择，但该回调无法将选中商品传回表单 → 无法为销售订单添加明细项。
- **修复**: 改为受控的 `<Modal>` 组件，在 `onOk` 回调中收集选中值。

#### F7 — 搜索页面中文列标题写死

- **文件**: `frontend/src/features/search/pages/SearchPage.tsx:10-44`
- **问题**: 列标题（SKU、名称、分类、规格、单位、库存、库位、状态）为硬编码中文字符串，未使用 `useTranslation`。英文模式下标题仍为中文。
- **修复**: 使用 `t('columns.sku')` 等 i18n 调用。
...
3. **F6** — 商品选择模态框（无法添加明细）
4. **F7** — 搜索页面中文（国际化失败）
5. **B4** — 合同 update_total_amount soft-delete
6. **B6** — 库存盘点 is_match 逻辑
### Phase 3 — 中优先级

1. **F8** — API 类型不匹配
2. **F9** — rowKey 翻页问题
3. **F10** — 审批按钮共享 loading
4. **F11** — 日期类型
5. **B3** — 合同 cancelled 状态
6. **B5** — 金额精度
### Phase 4 — 低优先级
