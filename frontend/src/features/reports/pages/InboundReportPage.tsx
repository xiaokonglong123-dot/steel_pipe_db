...
export default function InboundReportPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['report-inbound', filter],
    queryFn: () => reportApi.getInboundSummary(filter),
  });

  const handleExport = async () => {
    try {
      const res = await dataIoApi.exportInbound(filter);
      ...
      a.href = url;
      a.download = `入库报表_${dayjs().format('YYYYMMDD')}.xlsx`;
      a.click();
  ...
  const columns: ColumnsType<InventoryReport> = [
    ...
    { title: '入库数量', dataIndex: 'total', key: 'total', render: (v) => v?.toLocaleString() },
    { title: '在库', dataIndex: 'in_stock', key: 'in_stock', render: (v) => v?.toLocaleString() },
  ];

  return (
    <Card title="入库报表" styles={{ body: { padding: 16 } }}>
