...
import { purchaseApi, PurchaseOrder } from '../api/purchaseApi';
...
export default function PurchaseOrderListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => purchaseApi.list(filter as Record<string, undefined>),
  });

  const approveMutation = useMutation({
    mutationFn: (id: string) => purchaseApi.approve(id) as Promise<unknown>,
    onSuccess: () => {
  ...
  const cancelMutation = useMutation({
    mutationFn: (id: string) => purchaseApi.cancel(id) as Promise<unknown>,
    onSuccess: () => {
  ...
  const columns: ColumnsType<PurchaseOrder> = [
    ...
    { title: '创建时间', dataIndex: 'created_at', key: 'created_at', width: 170 },
    {
      ...
      width: 160,
      render: (_, r) =>
        r.inbound_refs && r.inbound_refs.length > 0
          ? r.inbound_refs.map((ref) => (
              <div key={ref.inbound_id}>
                <Link to={`/inventory/inbound/${ref.inbound_id}`}>{ref.inbound_no}</Link>
              </div>
            ))
          : '-',
    },
    {
      ...
      render: (_, r) => (
        <Space size="small">
          <Link to={`/purchases/${r.id}`}>
            <Button type="link" size="small">
              查看
            </Button>
          </Link>
          {(r.status === 'draft' || r.status === 'pending') && (
            <Link to={`/purchases/${r.id}/edit`}>
  ...
  return (
    <Card title="采购订单" styles={{ body: { padding: 16 } }}>
