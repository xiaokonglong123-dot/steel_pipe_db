...
async function login(): Promise<void> { loading.value = true; try { const data = await post<{ readonly user: AuthUser; readonly access_token: string }>("/auth/login", form); auth.setAuth(data.access_token, data.user); await router.push("/") } catch (error) { ElMessage.error(error instanceof Error ? error.message : "登录失败") } finally { loading.value = false } }
</script>
