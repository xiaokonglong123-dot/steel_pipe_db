...
import { getSalesOrder, actSalesOrder, deleteSalesOrder, shipSalesOrder, type ShippedItem } from "@/api/sales"
...
import type { OrderAction, SalesOrderDetail, SalesOrderItem } from "@/types/order"
...
async function onAction(action: OrderAction): Promise<void> {
  if (action === "edit") {
    void router.push(`/sales-orders/${id}/edit`)
    return
  }
  if (action === "delete") {
  ...
  await actSalesOrder(id, action as Exclude<OrderAction, "edit" | "delete" | "ship">)
...
const shipLines = reactive<ShipLine[]>([])

function remainingOf(it: SalesOrderItem): string {
  return sub(it.quantity, it.shipped_qty)
}

function openShip(): void {
...
async function submitShip(): Promise<void> {
  ...
  for (const line of shipLines) {
    ...
    if (line.location_id == null) {
      ElMessage.warning(`请为「${line.item_name}」选择出库库位`)
      return
    }
    if (gt(line.quantity, line.remaining)) {
      ElMessage.warning(`「${line.item_name}」发货数量超过剩余数量 ${line.remaining}`)
      return
    }
    items.push({ item_id: line.item_id, location_id: line.location_id, quantity: line.quantity })
  }
  if (items.length === 0) {
    ElMessage.warning("请输入至少一行的发货数量")
    return
  }
  shipping.value = true
  try {
    await shipSalesOrder(id, items)
...
<template>
  <section class="page" v-loading="loading">
    <template v-if="order">
      <PageHeader :title="`销售订单 ${order.order_no}`">
      ...
      <el-card>
        <el-descriptions :column="3" border>
          ...
          <el-descriptions-item label="状态">
            <OrderStatusTag :status="order.status" variant="sales" />
      ...
      <el-card class="items">
        ...
        <el-table :data="detail!.items" border stripe size="small">
          ...
          <el-table-column label="单价" width="120" align="right">
            <template #default="{ row }"><MoneyText :value="(row as SalesOrderItem).unit_price" /></template>
          </el-table-column>
          <el-table-column label="行小计" width="140" align="right">
            <template #default="{ row }"><MoneyText :value="(row as SalesOrderItem).total_price" strong /></template>
          </el-table-column>
    ...
    <el-dialog v-model="shipDialog" title="发货出库" width="720px" destroy-on-close>
      <el-table :data="shipLines" border size="small">
        ...
        <el-table-column label="出库库位" min-width="220">
          <template #default="{ row }">
