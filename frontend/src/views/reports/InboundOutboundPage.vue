...
import { inboundOutbound, downloadCsv } from "@/api/reports"
import type { InboundOutboundRow } from "@/types/reports"

const rows = ref<InboundOutboundRow[]>([])
...
async function load(): Promise<void> {
  loading.value = true
  try {
    rows.value = [...(await inboundOutbound({
      ...(filters.item_id ? { item_id: Number(filters.item_id) } : {}),
...
function renderChart(): void {
  ...
  for (const r of rows.value) {
    ...
    if (r.change_type === "inbound") o.inbound += qty
    else if (r.change_type === "outbound") o.outbound += Math.abs(qty)
  }
...
async function exportCsv(): Promise<void> {
  ...
  try {
    await downloadCsv(`/reports/inbound-outbound?${params.toString()}`, "inbound_outbound.csv")
  } catch (error) {
...
<template>
  <section class="page">
    <PageHeader title="出入库明细" subtitle="按商品/日期范围查询出入库流水">
    ...
    <el-card>
      <el-form inline>
        <el-form-item label="商品 ID">
          <el-input v-model="filters.item_id" clearable style="width: 140px" placeholder="可选" />
        </el-form-item>
      ...
      <el-table :data="rows" v-loading="loading" border stripe>
        ...
        <el-table-column label="数量" align="right" width="120">
          <template #default="{ row }"><MoneyText :value="(row as InboundOutboundRow).quantity" kind="qty" /></template>
        </el-table-column>
        <el-table-column prop="location_id" label="库位 ID" width="100" />
        <el-table-column prop="ref_type" label="关联类型" width="100" />
        <el-table-column prop="ref_id" label="关联 ID" width="90" />
      </el-table>
    </el-card>
  </section>
</template>
