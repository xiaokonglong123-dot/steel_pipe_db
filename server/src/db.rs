...
impl Database {
    ...
    pub async fn get_inventory_records(
        ...
        if let Some(v) = pipe_id { where_parts.push("pipe_id = ?".to_string()); args.push(Box::new(v.to_string())); }
        if let Some(v) = op_type { where_parts.push("operation_type = ?".to_string()); args.push(Box::new(v.to_string())); }
        if let Some(v) = start { where_parts.push("operation_date >= ?".to_string()); args.push(Box::new(v.to_string())); }
        if let Some(v) = end { where_parts.push("operation_date <= ?".to_string()); args.push(Box::new(v.to_string())); }
        ...
        let records: Vec<InventoryRecord> = stmt.query_map(
            ...
            |row| Ok(InventoryRecord {
                id: Some(row.get(0)?), pipe_id: row.get(1)?, operation_type: row.get(2)?,
                quantity: row.get(3)?, operation_date: row.get(4)?, operator: row.get(5)?, remarks: row.get(6)?,
            }),
        )?.map(|r| r.map_err(DbError::from)).collect::<Result<Vec<_>>>()?;
        Ok(records)
    }
    ...
    pub async fn get_recent_records(&self, limit: usize) -> Result<Vec<InventoryRecord>> {
        ...
        let records: Vec<InventoryRecord> = stmt.query_map(params![limit as i64], |row| {
            Ok(InventoryRecord {
                id: Some(row.get(0)?), pipe_id: row.get(1)?, operation_type: row.get(2)?,
                quantity: row.get(3)?, operation_date: row.get(4)?, operator: row.get(5)?, remarks: row.get(6)?,
            })
    ...
    pub async fn import_pipes_from_csv(&self, csv_content: &str, operator: &str) -> Result<(usize, usize)> {
        ...
        for (i, line) in csv_content.lines().enumerate() {
            ...
            let pipe = SteelPipe { id: None, pipe_id: pipe_id.clone(), diameter, thickness, length, material, quantity, location, supplier, entry_date: String::new(), last_update: None, status: "在库".to_string() };
            if self.add_pipe(&pipe).await.is_ok() {
                let record = InventoryRecord { id: None, pipe_id: pipe_id.clone(), operation_type: "入库".to_string(), quantity, operation_date: String::new(), operator: operator.to_string(), remarks: Some("批量导入".to_string()) };
                let _ = self.add_inventory_record(&record).await;
                success += 1;
            } else { fail += 1; }
        }
        Ok((success, fail))
    ...
    pub async fn export_inventory_csv(&self) -> Result<String> {
        let (pipes, _) = self.get_pipes(1, 100000, None, None, None, None, None, None, None).await?;
        let mut csv = String::from("钢管编号,直径(mm),壁厚(mm),长度(m),材质,数量,存放位置,供应商,入库日期,状态\n");
        for p in pipes {
            csv.push_str(&format!("{},{:.2},{:.2},{:.2},{},{},{},{},{},{}\n",
                p.pipe_id, p.diameter, p.thickness, p.length, p.material, p.quantity,
                p.location.as_deref().unwrap_or(""), p.supplier.as_deref().unwrap_or(""),
                p.entry_date, p.status));
        }
        Ok(csv)
    }

    pub async fn export_records_csv(&self, pipe_id: Option<&str>, op_type: Option<&str>, start: Option<&str>, end: Option<&str>) -> Result<String> {
        let records = self.get_inventory_records(pipe_id, op_type, start, end).await?;
        let mut csv = String::from("钢管编号,操作类型,数量,操作日期,操作员,备注\n");
        for r in records {
            csv.push_str(&format!("{},{},{},{},{},{}\n",
                r.pipe_id, r.operation_type, r.quantity, r.operation_date, r.operator,
    ...
    pub async fn import_pipes_from_excel(&self, excel_path: &str, operator: &str) -> Result<(usize, usize)> {
        ...
        if let Some(Ok(range)) = sheet {
            for (i, row) in range.rows().enumerate() {
                ...
                let supplier = if fields.len() > 7 && !fields[7].trim().is_empty() { Some(fields[7].trim().to_string()) } else { None };
                let pipe = SteelPipe { id: None, pipe_id: pipe_id.clone(), diameter, thickness, length, material, quantity, location, supplier, entry_date: String::new(), last_update: None, status: "在库".to_string() };
                if self.blocking_add_pipe(&pipe).is_ok() {
                    let record = InventoryRecord { id: None, pipe_id: pipe_id.clone(), operation_type: "入库".to_string(), quantity, operation_date: String::new(), operator: operator.to_string(), remarks: Some("批量导入".to_string()) };
                    let _ = self.blocking_add_inventory_record(&record);
                    success += 1;
                } else { fail += 1; }
            }
        }
    ...
    pub async fn export_inventory_to_excel(&self, path: &str) -> Result<()> {
        ...
        worksheet.write_string(0, 0, "钢管编号").ok();
        worksheet.write_string(0, 1, "直径(mm)").ok();
        worksheet.write_string(0, 2, "壁厚(mm)").ok();
        worksheet.write_string(0, 3, "长度(m)").ok();
        worksheet.write_string(0, 4, "材质").ok();
        worksheet.write_string(0, 5, "数量").ok();
        worksheet.write_string(0, 6, "存放位置").ok();
        worksheet.write_string(0, 7, "供应商").ok();
        worksheet.write_string(0, 8, "入库日期").ok();
        worksheet.write_string(0, 9, "状态").ok();
        for (i, p) in pipes.iter().enumerate() {
            let row = i as u32 + 1;
            worksheet.write_string(row, 0, &p.pipe_id).ok();
            worksheet.write_number(row, 1, p.diameter).ok();
            worksheet.write_number(row, 2, p.thickness).ok();
            worksheet.write_number(row, 3, p.length).ok();
            worksheet.write_string(row, 4, &p.material).ok();
            worksheet.write_number(row, 5, p.quantity as f64).ok();
            worksheet.write_string(row, 6, p.location.as_deref().unwrap_or("")).ok();
            worksheet.write_string(row, 7, p.supplier.as_deref().unwrap_or("")).ok();
            worksheet.write_string(row, 8, &p.entry_date).ok();
            worksheet.write_string(row, 9, &p.status).ok();
        }
    ...
    pub async fn export_records_to_excel(&self, path: &str, pipe_id: Option<&str>, op_type: Option<&str>, start: Option<&str>, end: Option<&str>) -> Result<()> {
        ...
        let worksheet = workbook.add_worksheet();
        worksheet.write_string(0, 0, "钢管编号").ok();
        worksheet.write_string(0, 1, "操作类型").ok();
        worksheet.write_string(0, 2, "数量").ok();
        worksheet.write_string(0, 3, "操作日期").ok();
        worksheet.write_string(0, 4, "操作员").ok();
        worksheet.write_string(0, 5, "备注").ok();
        for (i, r) in records.iter().enumerate() {
            let row = i as u32 + 1;
            worksheet.write_string(row, 0, &r.pipe_id).ok();
            worksheet.write_string(row, 1, &r.operation_type).ok();
            worksheet.write_number(row, 2, r.quantity as f64).ok();
            worksheet.write_string(row, 3, &r.operation_date).ok();
            worksheet.write_string(row, 4, &r.operator).ok();
            worksheet.write_string(row, 5, r.remarks.as_deref().unwrap_or("")).ok();
        }
        workbook.save(path).map_err(|e| DbError::Validation(e.to_string()))?;
        Ok(())
    }
