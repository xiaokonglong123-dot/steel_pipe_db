...
impl OutboundRepo {
    ...
    pub async fn create_with_items(
        ...
        for item in &dto.items {
            sqlx::query(
                "INSERT INTO outbound_items (outbound_id, item_id, quantity) VALUES (?, ?, ?)",
            )
            .bind(record.id)
            .bind(item.item_id)
            .bind(item.quantity)
    ...
    pub async fn find_by_id(pool: &SqlitePool, id: i64) -> Result<Option<OutboundRecord>, sqlx::Error> {
    ...
    pub async fn find_items(pool: &SqlitePool, outbound_id: i64) -> Result<Vec<OutboundItem>, sqlx::Error> {
        sqlx::query_as::<_, OutboundItem>(
            "SELECT id, outbound_id, item_id, quantity, created_at \
    ...
    pub async fn find_by_order_id(pool: &SqlitePool, order_id: i64) -> Result<Vec<OutboundRecord>, sqlx::Error> {
        sqlx::query_as::<_, OutboundRecord>(
            "SELECT id, outbound_no, outbound_type, order_id, customer_id, notes, approval_status, \
             rejection_reason, approval_reason, handled_by, handled_at, created_at, updated_at, deleted_at \
