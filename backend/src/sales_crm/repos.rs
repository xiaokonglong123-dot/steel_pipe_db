...
impl ShipmentRepo {
    ...
    pub async fn update_status(
        ...
        sqlx::query_as::<_, SalesShipment>(
            "UPDATE sales_shipments SET status = ?, \
                    shipped_at = CASE WHEN ? = 'shipped' AND shipped_at IS NULL THEN datetime('now') ELSE shipped_at END, \
    ...
    pub async fn list(pool: &SqlitePool, tenant_id: i64, sales_order_id: Option<i64>) -> Result<Vec<SalesShipment>, sqlx::Error> {
        sqlx::query_as::<_, SalesShipment>(
            "SELECT id, tenant_id, shipment_no, sales_order_id, shipped_at, carrier, \
                    tracking_no, status, items_json, notes, created_by, created_at, updated_at \
             FROM sales_shipments WHERE tenant_id = ? \
             AND (? IS NULL OR sales_order_id = ?) ORDER BY id DESC LIMIT 500",
        )
        .bind(tenant_id)
        .bind(sales_order_id)
        .bind(sales_order_id)
...
impl SalesQuoteRepo {
    ...
    pub async fn find_by_id(pool: &SqlitePool, tenant_id: i64, id: i64) -> Result<Option<SalesQuote>, sqlx::Error> {
        sqlx::query_as::<_, SalesQuote>(
            "SELECT id, tenant_id, quote_no, customer_id, quote_date, valid_until, \
                    total_amount, status, items_json, notes, created_at, updated_at, deleted_at \
             FROM sales_quotes WHERE tenant_id = ? AND id = ? AND deleted_at IS NULL",
        )
        .bind(tenant_id)
        .bind(id)
        .fetch_optional(pool)
    ...
    pub async fn update_status(
        ...
        sqlx::query_as::<_, SalesQuote>(
            "UPDATE sales_quotes SET status = ?, updated_at = datetime('now') \
    ...
    pub async fn list(pool: &SqlitePool, tenant_id: i64, customer_id: Option<i64>) -> Result<Vec<SalesQuote>, sqlx::Error> {
        sqlx::query_as::<_, SalesQuote>(
            "SELECT id, tenant_id, quote_no, customer_id, quote_date, valid_until, \
                    total_amount, status, items_json, notes, created_at, updated_at, deleted_at \
             FROM sales_quotes WHERE tenant_id = ? AND deleted_at IS NULL \
             AND (? IS NULL OR customer_id = ?) ORDER BY id DESC LIMIT 500",
        )
        .bind(tenant_id)
        .bind(customer_id)
        .bind(customer_id)
    ...
    pub async fn convert_to_order(
        ...
        let order_id: i64 = sqlx::query_scalar(
            "INSERT INTO sales_orders (order_no, customer_id, order_date, total_amount, status) \
             VALUES (?, ?, datetime('now'), ?, 'draft') RETURNING id",
        )
        .bind(order_no)
        ...
        .fetch_one(pool)
        .await?;
        // Copy quote items into sales_order_items (item_id/qty/unit_price/total_price).
        if let Some(items) = quote.items_json.as_array() {
            for item in items {
                ...
                .bind(order_id)
                .bind(item.get("item_id").and_then(|v| v.as_i64()).unwrap_or(0))
    ...
    pub async fn credit(pool: &SqlitePool, tenant_id: i64, customer_id: i64) -> Result<CustomerCredit, sqlx::Error> {
        sqlx::query_as::<_, CustomerCredit>(
            "SELECT ? AS customer_id, \
                    CAST(COALESCE((SELECT SUM(total_amount) FROM finance_invoices \
                              WHERE tenant_id = ? AND party_id = ? AND invoice_type = 'sales' \
                                AND status IN ('confirmed', 'draft')), 0.0) AS REAL) AS open_invoice_total, \
                    CAST(COALESCE((SELECT SUM(total_amount) FROM sales_orders \
                              WHERE customer_id = ? AND deleted_at IS NULL), 0.0) AS REAL) AS lifetime_sales",
        )
        .bind(customer_id)
        .bind(tenant_id)
        .bind(customer_id)
