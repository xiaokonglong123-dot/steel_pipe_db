...
impl AssetService {
    pub async fn create_asset(pool: &SqlitePool, tenant_id: i64, dto: &CreateAssetRequest) -> Result<FixedAsset, AppError> {
        ...
        let asset = FixedAsset {
            ...
            salvage_value: dto.salvage_value.unwrap_or(0.0),
    ...
    pub async fn list_assets(pool: &SqlitePool, tenant_id: i64, status: Option<&str>) -> Result<Vec<FixedAsset>, AppError> {
        AssetRepo::list(pool, tenant_id, status).await.map_err(AppError::from)
    }

    pub async fn get_asset(pool: &SqlitePool, tenant_id: i64, id: i64) -> Result<FixedAsset, AppError> {
        AssetRepo::find_by_id(pool, tenant_id, id)
            .await?
            .ok_or_else(|| AppError::NotFound(format!("Asset not found: {}", id)))
    }

    pub async fn update_asset(pool: &SqlitePool, tenant_id: i64, id: i64, dto: &UpdateAssetRequest) -> Result<FixedAsset, AppError> {
    ...
    pub async fn depreciate(pool: &SqlitePool, tenant_id: i64, id: i64, dto: &DepreciateRequest) -> Result<DepreciationEntry, AppError> {
        ...
        let monthly = (asset.purchase_cost - asset.salvage_value) / asset.useful_life_months as f64;
    ...
    pub async fn dispose_asset(pool: &SqlitePool, tenant_id: i64, id: i64) -> Result<FixedAsset, AppError> {
        let asset = Self::get_asset(pool, tenant_id, id).await?;
